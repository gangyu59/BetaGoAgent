...
function resizeBoard() {
    if (window.innerWidth <= 768) {
        // Mobile: fill screen width minus board-container padding (10px each side)
...
function drawBoard() {
    ...
    // Draw stars (hoshi) for 9x9: positions as [col, row]
    ...
    // Draw stones: server board[row][col], x=row, y=col
    ...
    // Draw last move marker: server sends last_move=[x, y] where x=row, y=col
...
function drawStone(col, row, color) {
    ...
    // Reset shadow before drawing to avoid leftover state
    ...
    // Second fill with shadow for 3D effect
    ...
    // Reset shadow so it doesn't bleed into next draw calls
    ctx.shadowColor = "transparent";
    ctx.shadowBlur = 0;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
...
function drawHeatmap(policy) {
    // policy: flat array of 81 floats, index = row * BOARD_SIZE + col
...
function updateStats(turn, analysis) {
    ...
    if (analysis) {
        // value: -1 (White wins) to +1 (Black wins) → 0..100% for Black
...
function updateScore(score) {
    ...
    if (blackEl) blackEl.innerText = score.black_total !== undefined ? score.black_total : 0;
    if (whiteEl) whiteEl.innerText = score.white_total !== undefined ? score.white_total : 7.5;
}
...
// Initial draw with responsive sizing
resizeBoard();
...
const lossChart = new Chart(ctxChart, {
    ...
    options: {
        ...
        scales: {
            ...
            yPolicy: { position: "left", beginAtZero: true },
            yValue: {
                position: "right",
                beginAtZero: true,
                suggestedMax: 0.005,
...
function updateTrainingChart(stats) {
    ...
    // Adjust value axis scale for fine details
    const newMax = Math.max(0.001, stats.value_loss * 3);
    ...
    if (wrSpan) wrSpan.innerText = (stats.win_rate * 100).toFixed(1);

    const lrSpan = document.getElementById("lr-val");
    if (lrSpan && stats.lr !== undefined) lrSpan.innerText = stats.lr.toFixed(6);
    ...
    if (stats.buffer !== undefined) {
        ...
        if (overlayEl) {
            ...
            if (msgEl) {
                msgEl.innerText = (stats.step > 0)
                    ? "Training running..."
                    : "Waiting for data to start training...";
            }
        }
    }
}
